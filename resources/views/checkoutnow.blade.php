...
<main class="max-w-[1360px] mx-auto px-16 my-10 mt-[137px]">
  <form action="{{route('create-now')}}" method="post" enctype="multipart/form-data">
    @csrf
  ...
  <div class="max-w-[960px] mx-auto border p-4 rounded-lg shadow-lg">
    ...
    <div class="">
          ...
          >
            ...
            <div class="grid grid-cols-12 gap-6 items-center py-4 border-b">
                ...
                <div class="col-span-6 flex items-center gap-4">
                    ...
                    <img src="{{$product->S_HinhAnh}}" alt="img-product" class="w-24">
                    <span class="">{{$product->S_Ten}}</span>
                    <!-- Thêm các trường thông tin khác nếu cần -->
                </div>
                <div class="col-span-2 font-semibold">{{ number_format($product->S_GiaBan, 0, ',', '.') }}đ</div>
                <div class="col-span-2 ml-3">
                    <span>{{ $productQuantity}}</span>
                </div>
                <div class="col-span-2 font-semibold" data-product-price ="  {{ number_format( $productQuantity * $product->S_GiaBan, 0, ',', '.') }}">
                  {{ number_format( $productQuantity * $product->S_GiaBan, 0, ',', '.') }}đ
                </div>
        ...
        <div class="flex flex-col items-end gap-4 my-6">
            <p>Tổng tiền: <span class="font-bold">  {{ number_format( $productQuantity * $product->S_GiaBan, 0, ',', '.') }}đ</span></p>
