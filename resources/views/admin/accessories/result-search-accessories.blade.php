        ...
        <div class="flex flex-wrap -mx-3 mb-10 mt-2">
          <div class="flex flex-col w-full max-w-full px-3">
            <div class="flex flex-col min-w-[980px] mb-6 bg-white border-0 shadow-md rounded-lg ">
              @if($results->count() == 0)
                <h2 class="ml-2 my-4 text-20 font-sora">Không tìm thấy kết quả phù hợp cho "{{$accessories_name}}"</h2>
              @else
              <div class="flex p-2 py-4 items-center justify-between">
                <h3 class="text-[#344767] text-20 font-sora">Kết quả tìm kiếm cho "{{ $accessories_name }}"</h3>
              </div>
                  ...
                  <table class="items-center w-full mb-0 align-top border-collapse text-slate-500">
                    ...
                    @foreach($results as $key => $value)
                      <tr class="border-t even:bg-gray-100 odd:bg-white ">
                        ...
                        <td class="p-4 bg-transparent">
                          <div class="px-2 py-1">
                            <h6 class="mb-0 text-sm leading-normal">{{ $value->accessories_type_name }}</h6>
                          </div>
                        </td>
                        ...
                        <td class="p-4 bg-transparent">
                          <div class="px-2 py-1">
                            <h6 class="mb-0 text-sm leading-normal">{{ number_format($value->accessories_price, 0, ',', '.') }}</h6>
                          </div>
                        </td>

                        <td class="flex bg-transparent mt-4 justify-center items-center">
                          <a href="accessories/{{$value->accessories_id}}/edit" class="text-16 mr-2 text-blue-100">
                            <i class="fa-regular fa-pen-to-square mr-2"></i>
                          </a>
                          <button class="delete-accessories text-16 mr-2 text-red-300 cursor-pointer" data-id="{{$value->accessories_id}}">
